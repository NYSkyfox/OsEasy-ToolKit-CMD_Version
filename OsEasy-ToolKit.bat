...
try {
    $null = $proc[0].MainModule;
    exit 1;
} catch {
    try { $proc[0].Resume(); } catch {}
    exit 0;
}"
...
'@;
$hwnd = [WinAPI]::FindWindow(null, 'OsEasy-ToolKit v1.0');
if ($hwnd -eq [IntPtr]::Zero) {
    $procs = Get-Process -Name 'cmd' -ErrorAction SilentlyContinue;
    foreach ($p in $procs) {
        if ($p.MainWindowHandle -ne [IntPtr]::Zero) {
            [WinAPI]::ShowWindowAsync($p.MainWindowHandle, 2);
            break;
        }
    }
} else {
...
if ($hwnd -eq [IntPtr]::Zero) {
    ...
    foreach ($p in $procs) {
        if ($p.MainWindowHandle -ne [IntPtr]::Zero) {
            [WinAPI]::ShowWindowAsync($p.MainWindowHandle, 1);
...
:check_service
set "svc_name=%~1"
for /f "tokens=4" %%a in ('sc query "%svc_name%" 2^>nul ^| findstr /C:"STATE"') do (
    if /i "%%a"=="RUNNING" (
        echo [运行中] %svc_name%
    ) else (
        echo [已停止] %svc_name%
